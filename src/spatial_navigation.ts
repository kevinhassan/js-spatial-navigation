...
interface ISpatialNavigation {
     ...
     */
    add(sectionId?: string, config?: IConfiguration): void;
...
export default function (): ISpatialNavigation {
    'use strict';
    ...
    function getRect(element: HTMLElement): IExtendedDOMRect {
        const { left, top, right, bottom, height, width } = element.getBoundingClientRect();
    ...
    function extend(out?: Object, ...elements: Object[]) {
        ...
        for (let i = 1; i < elements.length; i++) {
            ...
            for (const key in elements[i]) {
                if (elements[i].hasOwnProperty(key) && elements[i][key] !== undefined) {
    ...
    function fireEvent(elem: any, type: FiredEvents, details?: any, cancelable = true) {
    ...
    function focusSection(sectionId?: string) {
        const range: any = [];
        const addRange = function (id: any) {
    ...
    function focusNext(direction: Direction, currentFocusedElement: any, currentSectionId: any) {
        ...
        const config = extend({}, GLOBAL_CONFIG, _sections[currentSectionId]);
        let next;
    ...
    return {
        ...
        move: function (direction: Direction, selector: Selector) {
            ...
            const willmoveProperties: IWillMoveProperties = {
                direction: direction,
                sectionId: sectionId,
                cause: 'api',
            };
